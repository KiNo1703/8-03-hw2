...
## Вертикальный шардинг (Разделение по ответственности)
...
## Горизонтальный шардинг (Разделение по объему данных)
...
Приложение сначала обращается к нужному серверу на основе вертикального разделения (запросы к пользователям идут на один сервер, к книгам/магазинам — на другой).
Если запрос связан с таблицей books, система обращается к Конфигуратору шардов, чтобы определить физическое местоположение данных на основе shop_id.
Запрос перенаправляется на конкретный Product Shard, где и выполняется.

## Режимы работы серверов
User Profile DB (Сервер 1):
Режим: Master-сервер в режиме "Master-Slave" репликации.
Пояснение: Так как данные пользователей критичны, у этого сервера должен быть один или несколько Slave-реплик для отказоустойчивости и чтения (например, для генерации отчетов).

Product & Commerce DB (Сервер 2) и все Product Shards:
Режим: Каждый шард работает в своем независимом кластере "Master-Slave" или "Master-Master".
Пояснение: Каждый шард — это, по сути, независимая база данных. Для обеспечения доступности и масштабирования чтения каждый из них настраивается по схеме "Master-Slave". Запись идет только в мастер шарда, а чтение можно распределять на его слейвы.
